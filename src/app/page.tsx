...
type Message = { sender: "bot" | "user" | "system"; text: string };
...
export default function HomePage() {
  ...
  // persist session id so backend session is reused
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      const payload: any = { user_message: userText };
      ...
      if (data.session_id) {
        try { localStorage.setItem("arogya_session_id", data.session_id); } catch {}
        setSessionId(data.session_id);
      }

      // support multiple possible shapes from backend
      const assistantOutput = data.assistant_output ?? data.assistantOutput ?? data.assistant ?? {};
      const botText = (assistantOutput.content ?? assistantOutput.text ?? data.response ?? "Sorry, I didn't get a reply.") as string;
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl h-[80vh] flex flex-col overflow-hidden">
            ...
            <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-100">
              {messages.map((msg, index) => (
                <div key={index} className={`flex ${msg.sender === "user" ? "justify-end" : "justify-start"}`}>
                  <div className={`rounded-xl p-3 max-w-xs break-words ${msg.sender === "user" ? "bg-emerald-500 text-white rounded-br-none" : "bg-gray-300 text-gray-800 rounded-bl-none"}`}>
                    {msg.text}
                  </div>
                </div>
              ))}
              ...
              {isTyping && (
                <div className="flex justify-start">
                  <div className="rounded-xl p-3 max-w-xs bg-gray-300 text-gray-800 rounded-bl-none">
                    Typing...
                  </div>
                </div>
              )}
            ...
            <form onSubmit={handleSend} className="p-4 bg-white border-t border-gray-200">
              <div className="flex items-center space-x-2">
                ...
                />
                <button type="submit" className="bg-emerald-500 hover:bg-emerald-600 text-white p-3 rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-emerald-500">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
